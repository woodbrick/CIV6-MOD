...
function RefreshResources()
	...
	if (localPlayerID ~= -1) then
		...
		local pAmenities = Locale.Lookup("LOC_TOP_AMENITIES") .. "[NEWLINE]";
		...
		for resource in GameInfo.Resources() do
			...
			elseif (resource.ResourceClassType == "RESOURCECLASS_LUXURY") then
				local amount = pPlayerResources:GetResourceAmount(resource.ResourceType);
				if (amount > 0) then
					pAmenities = pAmenities .. "[NEWLINE][ICON_"..resource.ResourceType.."] " .. amount .. " " .. Locale.Lookup(resource.Name);
				end
			end
		end
		...
		Controls.PlayerAmenities:SetToolTipString(pAmenities);
	end
end
...
function RefreshAll()
	...
	Controls.LuxIcon:SetIcon("ICON_CIVILOPEDIA_RESOURCES");
end
