...
-- ===========================================================================
function HB_WorldInput_Check(pInputStruct:table)
	local minix, miniy = GetMinimapMouseCoords( pInputStruct:GetX(), pInputStruct:GetY() );
	if IsMouseInMinimap( minix, miniy ) then
		LuaEvents.WorldInput_HB_SetInMiniMap(true);
	else
		LuaEvents.WorldInput_HB_SetInMiniMap(false);
	end
end
...
function Initialize()
    ...
    --added by HB for mini map check
    LuaEvents.MiniMapPanel_HB_WorldInput_Check.Add(HB_WorldInput_Check);
end
Initialize();
