...
function OnLeaderClicked(playerID : number )
	...
	if playerID == localPlayerID then
		LuaEvents.OpenCivilopedia(civType);
	elseif Players[localPlayerID]:GetDiplomacy():HasMet(playerID) then
...
function OnLeaderRightClicked(ms_SelectedPlayerID : number )
	...
	local pPlayerConfig = PlayerConfigurations[ms_LocalPlayerID];
	local leaderTypeName = pPlayerConfig:GetLeaderTypeName();
	
	if ms_LocalPlayerID == ms_SelectedPlayerID then
		LuaEvents.OpenCivilopedia(leaderTypeName);
	elseif (not (relationshipHash == DiplomaticStates.WAR)) then
...
function AddLeader(iconName : string, playerID : number, isUniqueLeader: boolean)
	...
	-- HB: Set score values for DRS display
	instance.HB_ScoreOverall:SetText("[ICON_Capital]"..Players[playerID]:GetScore());
	instance.HB_MilitaryStrength:SetText("[ICON_Strength]"..Players[playerID]:GetStats():GetMilitaryStrengthWithoutTreasury());
			...
			if GameConfiguration.IsAnyMultiplayer() and isHuman then
				if(playerID ~= localPlayerID and not Players[localPlayerID]:GetDiplomacy():HasMet(playerID)) then
					...
					if gameEras:HasHeroicGoldenAge(playerID) then
						instance.EraLabel:SetText("[ICON_GLORY_SUPER_GOLDEN_AGE]");
					elseif gameEras:HasGoldenAge(playerID) then
						instance.EraLabel:SetText("[ICON_GLORY_GOLDEN_AGE]");
					elseif gameEras:HasDarkAge(playerID) then
						instance.EraLabel:SetText("[ICON_GLORY_DARK_AGE]");
					else
						instance.EraLabel:SetText("[ICON_GLORY_NORMAL_AGE]");
					end
				end
			else
