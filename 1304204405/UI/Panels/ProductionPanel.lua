...
--HB Members
local HB_INSTANCE_Y :number = 32;
local HB_ProductionQueue :boolean = true;
function HB_OnSettingsUpdate()
  HB_INSTANCE_Y = GameConfiguration.GetValue("HB_ProductionItemHeight");
  HB_ProductionQueue = GameConfiguration.GetValue("HB_ProductionQueue");
  if(not HB_ProductionQueue) then
    ResetAllCityQueues();
...
LuaEvents.HB_SettingsUpdate.Add(HB_OnSettingsUpdate);
LuaEvents.HB_SettingsInitialized.Add(HB_OnSettingsUpdate);
...
local HB_previousProductionHash :table = {};
local HB_currentProductionHash :table = {};
...
-- ====================HB Cityview==========================================
  ...
  function HB_OnCityviewEnabled()
    --print("OPEN PRODUCTION PANEL");
    Open();
  end
  
  function HB_OnCityviewDisabled()
    --print("CLOSE PRODUCTION PANEL");
    Close();
  end
  
  LuaEvents.HB_ProductionPanel_CityviewEnable.Add( HB_OnCityviewEnabled);
  LuaEvents.HB_ProductionPanel_CityviewDisable.Add( HB_OnCityviewDisabled);
...
function OnExpand(instance:table)
  ...
  Controls.ProductionList:ReprocessAnchoring();
  Controls.ProductionListScroll:CalculateInternalSize();
  if(not UI.GetInterfaceMode() == InterfaceModeTypes.BUILDING_PLACEMENT) or ( not UI.GetInterfaceMode() == InterfaceModeTypes.DISTRICT_PLACEMENT) then
    UI.SetInterfaceMode(InterfaceModeTypes.CITY_MANAGEMENT); -- only do this if NOT in district placement mode
  end
end
...
function BuildBuilding(city, buildingEntry)
  ...
  if ( bNeedsPlacement ) then     
    --If we were already placing something, quickly pop into selection mode, signalling to HB cityview code that placement was interrupted and resetting the view
...
function Close()
  if (Controls.SlideIn:IsStopped()) then      -- Need to check to make sure that we have not already begun the transition before attempting to close the panel.
    ...
    --print("prodQueue: " .. tostring(#prodQueue[selectedCity:GetID()]));
    --print("m_TypeNames: " .. tostring(#m_TypeNames));
    --print("HB_previousProductionHash: " .. tostring(#HB_previousProductionHash));
    --print("HB_currentProductionHash: " .. tostring(#HB_currentProductionHash));
    --print("m_kProductionQueueDropAreas: " .. tostring(#m_kProductionQueueDropAreas));
...
--  Close via click
function OnClose()
  LuaEvents.HB_CityPanel_CityviewDisable();
end
...
function View(data)
  ...
  --print(string.format("End Time: elapsed time: %.2f\n", os.clock() - x2))
  --print("End Time: " .. x2);
end
...
--HB modified function, puts everything in 1 unified list instead of 3
function PopulateList(data, listIM)
  ...
  local HB_ProdTable = {}; --Keeps track of each producable item. Key is the item hash, Value is a table with three keys (time/gold/faith) representing the respective costs
  local HB_PlayerGold = Players[Game.GetLocalPlayer()]:GetTreasury():GetGoldBalance();
  local HB_PlayerFaith = Players[Game.GetLocalPlayer()]:GetReligion():GetFaithBalance();
  ...
  for i, item in ipairs(unitPurchaseData) do
    if(HB_ProdTable[item.Hash] == nil) then
      HB_ProdTable[item.Hash] = {};
    end
    if (item.Yield == "YIELD_GOLD") then
      HB_ProdTable[item.Hash]["gold"] = item.Cost;
      if(item.Corps) then
        HB_ProdTable[item.Hash]["corpsGold"] = item.CorpsCost;
      end
      if(item.Army) then
        HB_ProdTable[item.Hash]["armyGold"] = item.ArmyCost;
      end
    else
      HB_ProdTable[item.Hash]["faith"] = item.Cost;
      if(item.Corps) then
        HB_ProdTable[item.Hash]["corpsFaith"] = item.CorpsCost;
      end
      if(item.Army) then
        HB_ProdTable[item.Hash]["armyFaith"] = item.ArmyCost;
      end
    end
  end
  for i, item in ipairs(unitData) do
      ...
      HB_ProdTable[item.Hash]["time"] = item.TurnsLeft;
      local unitListing;
      ...
      unitListing.ButtonContainer:SetSizeY(HB_INSTANCE_Y);
      ...
      unitListing.TrainUnit:RegisterCallback( Mouse.eLClick, function()
        QueueUnit(data.City, item, (m_isCONTROLpressed or not HB_ProductionQueue));
      end);
      ...
      --HB Productionpanel buy buttons
      ...
      if HB_ProdTable[item.Hash]["gold"] ~= nil then
        unitListing.PurchaseButton:SetText(HB_ProdTable[item.Hash]["gold"] .. "[ICON_GOLD]");
        unitListing.PurchaseButton:SetHide(false);
        unitListing.PurchaseButton:SetDisabled(HB_PlayerGold < HB_ProdTable[item.Hash]["gold"]);
        if (HB_PlayerGold < HB_ProdTable[item.Hash]["gold"]) then
          unitListing.PurchaseButton:SetColor(0xFF000000);
        else
          unitListing.PurchaseButton:SetColor(0xFFFFFFFF);
        end
      else
      ...
      if HB_ProdTable[item.Hash]["faith"] ~= nil then
        unitListing.FaithPurchaseButton:SetText(HB_ProdTable[item.Hash]["faith"] .. "[ICON_FAITH]");
        unitListing.FaithPurchaseButton:SetHide(false);
        unitListing.FaithPurchaseButton:SetDisabled(HB_PlayerFaith < HB_ProdTable[item.Hash]["faith"]);
        if (HB_PlayerFaith < HB_ProdTable[item.Hash]["faith"]) then
          unitListing.FaithPurchaseButton:SetColor(0xFF000000);
        else
          unitListing.FaithPurchaseButton:SetColor(0xFFFFFFFF);
        end
      else
      ...
      if item.Corps or item.Army then
        unitListing.ArmyCorpsDrawer:SetOffsetY(HB_INSTANCE_Y - 2);
        unitListing.CorpsButtonContainer:SetSizeY(HB_INSTANCE_Y);
        unitListing.CorpsPurchaseButton:SetSizeY(HB_INSTANCE_Y - 9);
        unitListing.CorpsFaithPurchaseButton:SetSizeY(HB_INSTANCE_Y - 9);
        unitListing.ArmyButtonContainer:SetSizeY(HB_INSTANCE_Y);
        unitListing.ArmyPurchaseButton:SetSizeY(HB_INSTANCE_Y - 9);
        unitListing.ArmyFaithPurchaseButton:SetSizeY(HB_INSTANCE_Y - 9);
      ...
      if item.Corps then  
        ...
        --HB Button binds
        if(HB_ProdTable[item.Hash]["corpsGold"] ~= nil) then
          unitListing.CorpsPurchaseButton:SetText(HB_ProdTable[item.Hash]["corpsGold"] .. "[ICON_GOLD]");
          unitListing.CorpsPurchaseButton:SetHide(false);
          unitListing.CorpsPurchaseButton:SetDisabled(HB_PlayerGold < HB_ProdTable[item.Hash]["corpsGold"]);
          if (HB_PlayerGold < HB_ProdTable[item.Hash]["corpsGold"]) then
            unitListing.CorpsPurchaseButton:SetColor(0xFF000000);
          else
            unitListing.CorpsPurchaseButton:SetColor(0xFFFFFFFF);
          end
        else
          unitListing.CorpsPurchaseButton:SetHide(true);
        end
        if(HB_ProdTable[item.Hash]["corpsFaith"] ~= nil) then
          unitListing.CorpsFaithPurchaseButton:SetText(HB_ProdTable[item.Hash]["corpsFaith"] .. "[ICON_FAITH]");
          unitListing.CorpsFaithPurchaseButton:SetHide(false);
          unitListing.CorpsFaithPurchaseButton:SetDisabled(HB_PlayerFaith < HB_ProdTable[item.Hash]["corpsFaith"]);
          if (HB_PlayerFaith < HB_ProdTable[item.Hash]["corpsFaith"]) then
            unitListing.CorpsFaithPurchaseButton:SetColor(0xFF000000);
          else
            unitListing.CorpsFaithPurchaseButton:SetColor(0xFFFFFFFF);
          end
        else
        ...
        unitListing.TrainCorpsButton:RegisterCallback( Mouse.eLClick, function()
          QueueUnitCorps(data.City, item, not HB_ProductionQueue);
        end);
      ...
      if item.Army then
        ...
        --HB Button binds
        if(HB_ProdTable[item.Hash]["armyGold"] ~= nil) then
          unitListing.ArmyPurchaseButton:SetText(HB_ProdTable[item.Hash]["armyGold"] .. "[ICON_GOLD]");
          unitListing.ArmyPurchaseButton:SetHide(false);
          unitListing.ArmyPurchaseButton:SetDisabled(HB_PlayerGold < HB_ProdTable[item.Hash]["armyGold"]);
          if (HB_PlayerGold < HB_ProdTable[item.Hash]["armyGold"]) then
            unitListing.ArmyPurchaseButton:SetColor(0xFF000000);
          else
            unitListing.ArmyPurchaseButton:SetColor(0xFFFFFFFF);
          end
        else
          unitListing.ArmyPurchaseButton:SetHide(true);
        end
        if(HB_ProdTable[item.Hash]["armyFaith"] ~= nil) then
          unitListing.ArmyFaithPurchaseButton:SetText(HB_ProdTable[item.Hash]["armyFaith"] .. "[ICON_FAITH]");
          unitListing.ArmyFaithPurchaseButton:SetHide(false);
          unitListing.ArmyFaithPurchaseButton:SetDisabled(HB_PlayerFaith < HB_ProdTable[item.Hash]["armyFaith"]);
          if (HB_PlayerFaith < HB_ProdTable[item.Hash]["armyFaith"]) then
            unitListing.ArmyFaithPurchaseButton:SetColor(0xFF000000);
          else
            unitListing.ArmyFaithPurchaseButton:SetColor(0xFFFFFFFF);
          end
        else
        ...
        unitListing.TrainArmyButton:RegisterCallback( Mouse.eLClick, function()
          QueueUnitArmy(data.City, item, not HB_ProductionQueue);
        end);
  ...
  prodUnitList = uL;
    ...
    if HB_previousProductionHash[selectedCity:GetID()] ~= nil then
      previousProductionHash = HB_previousProductionHash[selectedCity:GetID()];
    end
    ...
    for _,type in ipairs(m_TypeNames) do
      --print("Check: " .. BUILDING_IM_PREFIX..type);
    ...
    local selectedCity  = UI.GetHeadSelectedCity();
    if (cityNumDistricts[selectedCity:GetID()] == nil) or (cityNumDistricts[selectedCity:GetID()] < #data.DistrictItems) then
      cityNumDistricts[selectedCity:GetID()] = #data.DistrictItems;
      newDisType = true;
    end

    --print("Number of Districts: " .. tostring(cityNumDistricts[selectedCity:GetID()]));
    ...
    for i, item in ipairs(data.DistrictItems) do
      ...
      districtListing.ButtonContainer:SetSizeY(HB_INSTANCE_Y);
      districtListing.BuildingDrawer:SetOffsetY(HB_INSTANCE_Y);
      ...
      print("District Name: " .. item.Name);
      print("District Has Been Built: ", item.HasBeenBuilt);
      print("Num Per City: ", GameInfo.Districts[item.Type].OnePerCity);
      print("Needs Repair: ", item.Repair);
      print("Contaminated: ", item.Contaminated);
      print("Turns Left: ", item.TurnsLeft);
      print("District Complete: ", item.DistrictComplete);
      print("----------------------------------------------");
      ]]

      if(item.DistrictComplete and GameInfo.Districts[item.Type].OnePerCity == true and not item.Repair and not item.Contaminated) then
      ...
      districtListing.Button:RegisterCallback( Mouse.eLClick, function()
        if(m_isCONTROLpressed or not HB_ProductionQueue) then
      ...
      print("GoldAllowed: ", item.GoldAllowed);
      print("GoldCost: ", item.GoldCost);
      print("CantAfford: ", item.CantAfford);

      districtListing.PurchaseButton:RegisterCallback( Mouse.eLClick, function()
        PurchaseDistrict(data.City, item);
      end);

      if (item.GoldAllowed and (not item.Disabled)) then
        districtListing.PurchaseButton:SetText(item.GoldCost .. "[ICON_GOLD]");
        districtListing.PurchaseButton:SetHide(false);
        districtListing.PurchaseButton:SetDisabled(item.CantAfford);
        if (item.CantAfford) then
          districtListing.PurchaseButton:SetColor(0xFF000000);
        else
          districtListing.PurchaseButton:SetColor(0xFFFFFFFF);
        end
      else
    ...
    for i, buildingItem in ipairs(data.BuildingItems) do
      ...
      local pBuildQueue  = selectedCity:GetBuildQueue();
      local canBuildThis = pBuildQueue:CanProduce( buildingItem.Hash, true );
      local displayItem = pBuildQueue:CanProduce( buildingItem.Hash, true );

      --only need to check items in the queue if display items is true
      if displayItem == true then
          -- loop thru all entries in the MutuallyExclusiveBuildings checking for any entires of this hash and see if the city has that building
          for row in GameInfo.MutuallyExclusiveBuildings() do
            if (row.MutuallyExclusiveBuilding == buildingItem.Type) then
              if IsBuildingInQueue(selectedCity, row.Hash) then
                displayItem = false;
              end
            elseif (row.Building == buildingItem.Type) then
              if IsBuildingInQueue(selectedCity, GameInfo.Buildings[row.MutuallyExclusiveBuilding].Hash) then
                displayItem = false;
              end
            end
          end
      ...
      if (buildingItem.Hash == GameInfo.Buildings["BUILDING_PALACE"].Hash) then
        print("Checking Palace");
        print("Hash: " .. buildingItem.Hash);
        print("Can Produce: " .. tostring(canBuildThis));
      end

      if (buildingItem.Hash == GameInfo.Buildings["BUILDING_BASILIKOI_PAIDES"].Hash) then
        print("Checking Funky Named Barracks");
        print("Hash: " .. buildingItem.Hash);
        print("Can Produce: " .. tostring(canBuildThis));
      end

      if (buildingItem.Hash == GameInfo.Buildings["BUILDING_STABLE"].Hash) then
      ...
      if(buildingItem.Hash == GameInfo.Buildings["BUILDING_PALACE"].Hash) or (buildingItem.Display == false) then
        displayItem = false;
      end
      --]]
      ...
      if(not buildingItem.IsWonder and not IsBuildingInQueue(selectedCity, buildingItem.Hash) and displayItem) then
        ...
        --print("Building Name: " .. buildingItem.Name .. " In District: " .. uniqueIMName);
        if (districtList[uniqueIMName] ~= nil) then
          ...
          if(HB_ProdTable[buildingItem.Hash] == nil) then
            HB_ProdTable[buildingItem.Hash] = {};
          end
          HB_ProdTable[buildingItem.Hash]["time"] = buildingItem.TurnsLeft;
          ...
          --print("Drawer Name: " .. uniqueDrawerName);
          ...
          buildingListing.Button:RegisterCallback( Mouse.eLClick, function()
            QueueBuilding(data.City, buildingItem, not HB_ProductionQueue);
          end);
          ...
          if HB_ProdTable[buildingItem.Hash]["gold"] ~= nil then
            buildingListing.PurchaseButton:SetText(HB_ProdTable[buildingItem.Hash]["gold"] .. "[ICON_GOLD]");
            buildingListing.PurchaseButton:SetHide(false);
            buildingListing.PurchaseButton:SetDisabled(HB_PlayerGold < HB_ProdTable[buildingItem.Hash]["gold"]);
            if (HB_PlayerGold < HB_ProdTable[buildingItem.Hash]["gold"]) then
              buildingListing.PurchaseButton:SetColor(0xFF000000);
            else
              buildingListing.PurchaseButton:SetColor(0xFFFFFFFF);
            end
          else
          ...
          if HB_ProdTable[buildingItem.Hash]["faith"] ~= nil then
            buildingListing.FaithPurchaseButton:SetText(HB_ProdTable[buildingItem.Hash]["faith"] .. "[ICON_FAITH]");
            buildingListing.FaithPurchaseButton:SetHide(false);
            buildingListing.FaithPurchaseButton:SetDisabled(HB_PlayerFaith < HB_ProdTable[buildingItem.Hash]["faith"]);
            if (HB_PlayerFaith < HB_ProdTable[buildingItem.Hash]["faith"]) then
              buildingListing.FaithPurchaseButton:SetColor(0xFF000000);
            else
              buildingListing.FaithPurchaseButton:SetColor(0xFFFFFFFF);
            end
          else
          ...
          if (buildingItem.Disabled) then 
            ...
            buildingListing.ButtonContainer:SetSizeY(HB_INSTANCE_Y);
            buildingListing.Button:SetSizeY(HB_INSTANCE_Y);
            buildingListing.Button:SetColor(0xffffffff);
          end
          buildingListing.Button:SetDisabled(buildingItem.Disabled);

          --districtList[uniqueIMName].:CalculateSize();
        end
      end
    end
    ...
    for i, item in ipairs(data.BuildingItems) do
      if(item.IsWonder) then
        ...
        wonderListing.ButtonContainer:SetSizeY(HB_INSTANCE_Y);
        wonderListing.Button:SetSizeY(HB_INSTANCE_Y);
        ...
        wonderListing.Button:RegisterCallback( Mouse.eLClick, function()
            QueueBuilding(data.City, item, not HB_ProductionQueue);
          end);
    ...
    for i, item in ipairs(data.ProjectItems) do
      ...
      projectListing.ButtonContainer:SetSizeY(HB_INSTANCE_Y);
      if (item.Disabled) then 
        if(showDisabled) then
      ...
      projectListing.Button:RegisterCallback( Mouse.eLClick, function()
          QueueProject(data.City, item, not HB_ProductionQueue);
      end);
  ...
  --===================================================================================================================
    ...
    if(#prodQueue[cityID] > 0) then
      ...
      --print("Number of Items IN Queue: " .. #prodQueue[cityID]);
      ...
      for i, qi in pairs(prodQueue[cityID]) do
        ...
        Controls.ProductionQueueList:SetSizeY((32 * #prodQueue[cityID]));
      end
    end
  ...
  if CityManager.CanStartCommand( pCity, CityCommandTypes.PURCHASE, true, tParameters, false ) then
    ...
    if results ~= nil then
      ...
      if kUnit.Corps then
        ...
        --print("Print Cost1: " .. kUnit.CorpsCost);
...
function ComposeDistrictForPurchase( pRow:table, pCity:table, sYield:string, pYieldSource:table, sCantAffordKey:string )
  ...
  if CityManager.CanStartCommand( pCity, CityCommandTypes.PURCHASE, true, tParameters, false ) then
    ...
    return true, isCantAfford, pCity:GetGold():GetPurchaseCost( YIELD_TYPE, pRow.Hash ), sYield;
  end
  return false, nil;
end
...
function Refresh()
  ...
  if (selectedCity ~= nil) then
    ...
    if (cityOwner == playerID) then
      ...
      local currentProductionHash = buildQueue:GetCurrentProductionTypeHash();
      HB_currentProductionHash[cityID] = currentProductionHash;
      GameConfiguration.SetValue("HB_currentProductionHash" .. cityID, HB_currentProductionHash[cityID]);
...
--          print("------------------------------------");
  --        print("District: " .. row.DistrictType);
    --      print("Turns: " .. turnsLeft);
      --    print("Cost: " .. iProductionCost);
        --  print("Progress: " .. iProductionProgress);
          --print("------------------------------------");

          -- Can it be purchased with gold?
          local isAllowed, isCantAfford, sGoldCost, sYield = ComposeDistrictForPurchase( row, selectedCity, "YIELD_GOLD", playerTreasury, "LOC_BUILDING_INSUFFICIENT_FUNDS" );
      ...
      for row in GameInfo.Buildings() do
        ...
        if(not row.IsWonder) then
          ...
          --print("District: " .. prereqDistrict .. " Building: " .. row.Name);
          ...
          if((prereqDistrict and IsHashInQueue( selectedCity, prereqDistrict.Hash)) or cityDistricts:HasDistrict(prereqDistrict.Index, true)) then
            ...
            --print("Valid");
            --print("District Is Built: " .. row.BuildingType);
                ...
                if(GameInfo.Buildings[row.Hash].EnabledByReligion) then
                  ...
                  if ((table.count(cityData.Religions) > 1) or (cityData.PantheonBelief > -1)) then
                    ...
                    if cityData.PantheonBelief > -1 then
                      for bModifiers in GameInfo.BeliefModifiers() do
                        if (bModifiers.BeliefType == GameInfo.Beliefs[cityData.PantheonBelief].BeliefType) then
                          table.insert(modifierIDs, bModifiers.ModifierID);
                        end
                      end
                    end
        ...
        if not row.MustPurchase and ( buildQueue:CanProduce( row.Hash, true ) or (doShow and not row.IsWonder) ) then
          ...
          --print("Name: " .. row.BuildingType .. "CanStart: " .. tostring(isCanStart));

          if(row.IsWonder or not doShow) then
            isDisabled = not isCanStart;
            if (row.IsWonder and IsHashInAnyQueue(row.Hash)) then
              isDisabled = true;
            end
          end
          ...
          --print("District: " .. iPrereqDistrict);
          --print("Adding Building: " .. row.Name);
        ...
        if buildQueue:CanProduce( row.Hash, true ) then
          ...
          if results ~= nil then
            if results[CityOperationResults.CAN_TRAIN_CORPS] then
              ...
              --print("Print Cost2: " .. kUnit.CorpsCost);
      ...
      for row in GameInfo.Projects() do
        ...
        if not (row.ProjectType == "PROJECT_PLACEHOLDER") then
      ...
      --print(string.format("Mid Time: elapsed time: %.2f\n", os.clock() - x2))
      --print("Mid Time: " .. x2);
...
function OnCityProductionChanged(playerID:number, cityID:number)
  ...
  if (not HB_ProductionQueue) then --If production queue is disabled, clear out the queue
  ...
  HB_previousProductionHash[cityID] = HB_currentProductionHash[cityID];
  GameConfiguration.SetValue("HB_previousProductionHash" .. cityID, HB_previousProductionHash[cityID]);
end
...
function QueueDistrict(city, districtEntry, skipToFront)
  ...
  if (bNeedsPlacement) then
    ...
    print("Adding dosctrict to queue!");
    local tParameters = {};
    tParameters[CityOperationTypes.PARAM_DISTRICT_TYPE] = districtEntry.Hash;
    tParameters[CityOperationTypes.PARAM_INSERT_MODE] = CityOperationTypes.VALUE_EXCLUSIVE;
    local cityID = city:GetID();
          ...
          if(not buildQueue:CanProduce( qi.entry.Hash, true ) and canUpgrade and canBuildNewUnit) then
            ...
            if results ~= nil then
              if results[CityOperationResults.CAN_TRAIN_CORPS] then
                ...
                --print("Print Cost3: " .. kUnit.CorpsCost);
...
function ResizeQueueWindow()
  ...
  Controls.ProductionQueueListScroll:SetSizeY(windowHeight);
...
function Initialize()
  ...
  LuaEvents.ProductionPanel_CloseView.Add(Close);
  ...
  -- HB Setting Controls
  --PopulateCheckBox(Controls.HB_ProductionQueueCheckbox, "HB_ProductionQueue");
  --RegisterControl(Controls.HB_ProductionQueueCheckbox, "HB_ProductionQueue", UpdateCheckbox);

  -- HB Loading Previous Turn items
  -- ===================================================================
  ...
  if GameConfiguration.IsSavedGame() then
    ...
    for i=0,cityCount-1 do
      HB_currentProductionHash[i] = GameConfiguration.GetValue("HB_currentProductionHash" .. i);
    end

    for i=0,cityCount-1 do
      HB_previousProductionHash[i] = GameConfiguration.GetValue("HB_previousProductionHash" .. i);
    end
