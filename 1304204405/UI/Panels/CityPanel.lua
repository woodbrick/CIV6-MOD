...
-- ===========================================================================
function HB_CityviewEnableManager()
  HB_cityview = true;
  HB_wonderMode = false;
  LuaEvents.HB_ProductionPanel_CityviewEnable();
  LuaEvents.HB_CityPanel_CityviewEnable();
  LuaEvents.HB_CityPanelOverview_CityviewEnable();
  LuaEvents.HB_WorldInput_CityviewEnable();
  --print("HB_CityviewEnableManager");
end

-- ===========================================================================
function HB_CityviewDisableManager()
  HB_cityview = false;
  HB_wonderMode = false;
  LuaEvents.HB_ProductionPanel_CityviewDisable();
  LuaEvents.HB_CityPanel_CityviewDisable();
  LuaEvents.HB_CityPanelOverview_CityviewDisable();
  LuaEvents.HB_WorldInput_CityviewDisable();
end
...
LuaEvents.HB_CityPanel_CityviewEnable.Add( HB_OnCityviewEnabled);
LuaEvents.HB_CityPanel_CityviewDisable.Add( HB_OnCityviewDisabled);
LuaEvents.HB_CityviewDisable.Add( HB_CityviewDisableManager);
LuaEvents.HB_CityviewEnable.Add( HB_CityviewEnableManager);
LuaEvents.HB_Strike_Enter.Add (function() HB_usingStrikeButton = true; end)
LuaEvents.HB_Strike_Exit.Add (function() HB_usingStrikeButton = false; end)

-- ===========================================================================
function HB_OnInterfaceModeChanged( eOldMode:number, eNewMode:number )
  print("------------------ INTERFACE CHANGE ------------------");

  if(eOldMode == InterfaceModeTypes.CITY_MANAGEMENT) then
    print("Old Mode: CITY_MANAGEMENT");
  elseif(eOldMode == InterfaceModeTypes.DISTRICT_PLACEMENT) then
   --print("Old Mode: DISTRICT_PLACEMENT");
  elseif(eOldMode == InterfaceModeTypes.BUILDING_PLACEMENT) then
    --print("Old Mode: BUILDING_PLACEMENT");
  else
    --print("Old Mode: Uknown: " .. eOldMode);
  end

  if(eNewMode == InterfaceModeTypes.CITY_MANAGEMENT) then
    print("New Mode: CITY_MANAGEMENT");
  elseif(eNewMode == InterfaceModeTypes.DISTRICT_PLACEMENT) then
    --print("New Mode: DISTRICT_PLACEMENT");
  elseif(eNewMode == InterfaceModeTypes.BUILDING_PLACEMENT) then
    --print("New Mode: BUILDING_PLACEMENT");
  else
    --print("New Mode: Uknown: " .. eNewMode);
  end
  ...
  if(eOldMode == InterfaceModeTypes.CITY_MANAGEMENT or eOldMode == InterfaceModeTypes.DISTRICT_PLACEMENT or eOldMode == InterfaceModeTypes.BUILDING_PLACEMENT) then
    if(eNewMode == InterfaceModeTypes.DISTRICT_PLACEMENT or eNewMode == InterfaceModeTypes.BUILDING_PLACEMENT) then 
      --print("WonderMode");
      HB_WonderModeEnabled();
    elseif(eNewMode ~= InterfaceModeTypes.CITY_MANAGEMENT) then
      if(HB_wonderMode) then
        --print("View Enable 1");
        LuaEvents.HB_CityviewEnable();
      else
        --print("View Disable");
        LuaEvents.HB_CityviewDisable();
      end
    else
      --print("View Enable 2");
      LuaEvents.HB_CityviewEnable();
...
-- ===========================================================================
function HB_OnCitySelectionChanged( ownerPlayerID:number, cityID:number, i:number, j:number, k:number, isSelected:boolean, isEditable:boolean)
...
function OnInterfaceModeChanged( eOldMode:number, eNewMode:number )	
	...
	print("Interface Changed #CityPanel: " , eOldMode, eNewMode);
...
function Initialize()
	...
	Controls.NextCityButton:RegisterCallback(	Mouse.eLClick,	HB_OnNextCity); 
	Controls.PrevCityButton:RegisterCallback(	Mouse.eLClick,	HB_OnPreviousCity); 
	...
	-- HB hide buttons
	Controls.PurchaseTileCheck:SetHide(true);
	Controls.ManageCitizensCheck:SetHide(true);
	Controls.ProduceWithGoldCheck:SetHide(true);
	Controls.ProduceWithFaithCheck:SetHide(true);
	Controls.ChangeProductionCheck:SetHide(true);
	Controls.ToggleOverviewPanel:SetHide(true);

	-- Game Core Events
	Events.CityAddedToMap.Add(			OnCityAddedToMap );
	Events.CityNameChanged.Add(			OnCityNameChanged );
	Events.CitySelectionChanged.Add(	HB_OnCitySelectionChanged );
	...
	Events.InterfaceModeChanged.Add(	HB_OnInterfaceModeChanged );
	Events.LocalPlayerChanged.Add(		OnLocalPlayerChanged );
	Events.UnitSelectionChanged.Add(	OnUnitSelectionChanged );
	Events.PlayerResourceChanged.Add(	OnPlayerResourceChanged );

	-- LUA Events
	LuaEvents.CityPanel_ClosePanel.Add(Close);
	LuaEvents.CityPanel_OpenPanel.Add(Open);
