...
-- added by HB for minimap size scroll
local m_HB_inMiniMap				:boolean = false;

local HB_cityview = false;
-- ===========================================================================
--	FUNCTIONS
-- ===========================================================================
...
function OnDefaultChangeToSelectionMode( pInputStruct )
	LuaEvents.ProductionPanel_CloseView();
	UI.SetInterfaceMode(InterfaceModeTypes.SELECTION);
end
...
function OnMouseWheelZoom( pInputStruct:table )
	...
	LuaEvents.MiniMapPanel_HB_WorldInput_Check(pInputStruct);

	if (m_HB_inMiniMap == true) then
		wheelValue = (pInputStruct:GetWheel() / 2400)
		oldZoom = Options.GetGraphicsOption("General", "MinimapSize") or 0.0;
		newZoom = oldZoom + wheelValue;
		if (newZoom > 1) then newZoom = 1; end
		if (newZoom < 0) then newZoom = 0; end
		Options.SetGraphicsOption("General", "MinimapSize", newZoom)
		
		UI.SetMinimapSize(newZoom)
	else
...
function Initialize()
	...
	--added by HB for mini map scroll
	LuaEvents.WorldInput_HB_SetInMiniMap.Add(function(inMiniMap) m_HB_inMiniMap = inMiniMap; end);

	-- HB Events
  	LuaEvents.HB_WorldInput_CityviewEnable.Add( function() HB_cityview = true; end );
  	LuaEvents.HB_WorldInput_CityviewDisable.Add( function() HB_cityview = false; end );
