...
function CityBanner:UpdatePopulation(isLocalPlayer:boolean, pCity:table, pCityGrowth:table)
	...
	if isLocalPlayer then
		...
		--HB chack and track the population
		if (currentPopulation > self.hbPopSize) then
			LuaEvents.AddCustomNotification('NEWPOP', Game.GetLocalPlayer(), pCity:GetID(), pCity:GetName(), currentPopulation);
			print("City Just Grew");
		end
		
		self.hbPopSize = currentPopulation;
		print("City Pop Size: " .. self.hbPopSize);
	else
			...
			if (localPlayerID == iCityOwner) then
				...
				if (pBuildQueue ~= nil) then
					...
					if (currentProduction ~= nil) then
						...
						productionInstance.SlotMeter:SetHide(false);
						productionInstance.SlotMeter:SetPercent(1 - productionpct);
						...
						productionInstance.SlotMeter:SetHide(true);
		...
		if leader ~= nil then
			if GameInfo.CivilizationLeaders[leader] == nil then
				...
				if civType ~= nil then
					...
					local tooltip:string = Locale.Lookup("LOC_CITY_BELONGS_TT", pPlayerConfig:GetCivilizationShortDescription());
					...
					if pCulturalIdentity then
						...
						if conversionOutcome == IdentityConversionOutcome.GAINING_LOYALTY then
							...
							if (turnsToConversion > 0) then
								tooltip = tooltip .. "[NEWLINE]" .. Locale.Lookup("LOC_HUD_CITY_GAINING_LOYALTY", turnsToConversion);
							else
								tooltip = tooltip .. "[NEWLINE]" .. Locale.Lookup("LOC_HUD_CITY_FULL_LOYALTY");
							end
						elseif conversionOutcome == IdentityConversionOutcome.LOSING_LOYALTY then
							isLoyaltyFalling = true;
							tooltip = tooltip .. "[NEWLINE]" .. Locale.Lookup("LOC_HUD_CITY_LOSING_LOYALTY", turnsToConversion);
						else
							tooltip = tooltip .. "[NEWLINE]" .. Locale.Lookup("LOC_HUD_CITY_STABLE_LOYALTY");
						end
					end
	...
	if instance then
		...
		if pCity then
			...
			if pCityCulturalIdentity then
				...
				instance.OwnerCivIcon:SetToolTipString(Locale.Lookup(pOwnerConfig:GetCivilizationDescription()));
				...
				if transferPlayerID ~= -1 then
					...
					instance.TopCivIcon:SetToolTipString(Locale.Lookup(pTopConfig:GetCivilizationDescription()));
...
function OnInterfaceModeChanged( oldMode:number, newMode:number )
	...
	print("Interface Changed #CityBannerManager: " , oldMode, newMode);
	...
	if (oldMode == InterfaceModeTypes.DISTRICT_PLACEMENT) then
    	print("comeon2");
...
function Initialize()
	...
	-- add game event to keep banners refresh on citizen changes
	Events.CityWorkerChanged.Add(				RefreshBanners);
	...
	print("HB City Overview Interface");
end
Initialize();
