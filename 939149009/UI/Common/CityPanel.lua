...
function ViewMain( data:table )
	...
	Controls.RenameCityButton:RegisterCallback(Mouse.eLClick, function()
		Controls.CityName:SetHide(true);

		local sOld = Controls.CityName:GetText()

		Controls.EditCityName:SetText(data.IsCapital and titlecase(string.gsub(sOld, "%[ICON_Capital%]", "", 1)) or titlecase(sOld));
	...
	Controls.EditCityName:RegisterCommitCallback(function(editBox)
		...
		Controls.CityName:SetHide(false);
	end);	
	...
	Controls.EditCityName:SetColor(m_secondaryColor);
	...
	---Controls.FoodCheck:GetTextButton():SetText(			"[ICON_Food]"		..toPlusMinusString(data.FoodSurplus) );
	...
	-----------------------------------
	-- Food is weird: Logic from the CityPanelOverview
	-----------------------------------
	if data.FoodSurplus >= 0 then
		local iGrowthModifier =  math.max(1 + (data.HappinessGrowthModifier/100) + data.OtherGrowthModifiers, 0) -- This is unintuitive but it's in parity with the logic in City_Growth.cpp
		local iModifiedFood_A = Round(data.FoodSurplus * iGrowthModifier, 2)
		local iModifiedFood_H = iModifiedFood_A * data.HousingMultiplier

		Controls.FoodCheck:GetTextButton():SetText(			"[ICON_Food]"		..toPlusMinusString(iModifiedFood_H) )

		local tToolTip = {}
		table.insert(tToolTip, data.FoodPerTurnToolTip)
		table.insert(tToolTip, Locale.Lookup("LOC_SUK_CITY_YIELD_FROM_POPULATION_TOOLTIP", toPlusMinusString(-(data.FoodPerTurn - data.FoodSurplus)) ))
		table.insert(tToolTip, Locale.Lookup("LOC_SUK_CITY_YIELD_FROM_HAPPINESS_TOOLTIP", toPlusMinusString(iGrowthModifier * 100 - 100), toPlusMinusString(iModifiedFood_A - data.FoodSurplus) ))
		table.insert(tToolTip, Locale.Lookup("LOC_SUK_CITY_YIELD_FROM_HOUSING_TOOLTIP", toPlusMinusString(data.HousingMultiplier * 100 - 100), toPlusMinusString(iModifiedFood_H - iModifiedFood_A) ))

		local sFoodTT = table.concat(tToolTip, "[NEWLINE]");
		RealizeYield3WayCheck( data.YieldFilters[YieldTypes.FOOD], YieldTypes.FOOD, sFoodTT)
	else
		Controls.FoodCheck:GetTextButton():SetText(			"[ICON_Food]"		..toPlusMinusString(data.FoodSurplus))
	...
	local iNextDistrictPop = math.floor((data.Population - 1) / 3 + 1) * 3  + 1
	if not Modding.IsModActive("1B28771A-C749-434B-9053-D1380C553DE9") then
		...
		Controls.BreakdownNum:SetToolTipString(Locale.Lookup("LOC_SUK_DISTRICT_POPULATION_TOOLTIP", data.DistrictsPossibleNum, iNextDistrictPop))		
	else
		Controls.ReligionNum:SetText( data.DistrictsNum.."/"..data.DistrictsPossibleNum );
		Controls.ReligionLabel:SetText(Locale.ToUpper(Locale.Lookup("LOC_HUD_DISTRICTS")));
		Controls.ReligionNum:SetToolTipString(Locale.Lookup("LOC_SUK_DISTRICT_POPULATION_TOOLTIP", data.DistrictsPossibleNum, iNextDistrictPop))
...
-- LuaEvents.ToggleCityManagement.Add(function()
