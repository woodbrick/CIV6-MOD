...
function PopulateUnlockablesForCivic(playerID:number, civicID:number, kItemIM:table, kGovernmentIM:table, callback:ifunction, hideDescriptionIcon:boolean )
	...
	-- Suk Addin
	-----------------------------------------------------------------------
	local tParent = kItemIM.m_ParentControl:GetParent()
	-- if tParent:GetID() == "Top" or tParent:GetID() == "NodeButton" then
	if tParent:GetID() == "Top" then
		tParent:ClearMouseEnterCallback()
		tParent:RegisterMouseEnterCallback(			function() LuaEvents.Suk_RealiseEnhancedTechTooltip(playerID, civicData.CivicType) end)
		tParent:RegisterMouseExitCallback(			function() LuaEvents.Suk_HideEnhancedTechTooltip() end)
		tParent:ClearToolTipCallback()
	end
	-----------------------------------------------------------------------
	-----------------------------------------------------------------------
...
function PopulateUnlockablesForTech(playerID:number, techID:number, instanceManager:table, callback:ifunction )
	...
	-- Suk Addin
	-----------------------------------------------------------------------
	local tParent = instanceManager.m_ParentControl:GetParent()
	-- if tParent:GetID() == "Top" or tParent:GetID() == "NodeButton" then
	if tParent:GetID() == "Top" then
		tParent:ClearMouseEnterCallback()
		tParent:RegisterMouseEnterCallback(			function() LuaEvents.Suk_RealiseEnhancedTechTooltip(playerID, techType); end);
		tParent:RegisterMouseExitCallback(			function() LuaEvents.Suk_HideEnhancedTechTooltip(); end);
		tParent:ClearToolTipCallback()
	end
	-----------------------------------------------------------------------
...
function RealizeCurrentCivic( playerID:number, kData:table, kControl:table, cachedModifiers:table )
	...
	if kData ~= nil then
		...
		kControl.MainPanel:RegisterMouseEnterCallback(			function() kControl.MainGearAnim:Play(); LuaEvents.Suk_RealiseEnhancedTechTooltip(playerID, techType); end);
		kControl.MainPanel:RegisterMouseExitCallback(			function() kControl.MainGearAnim:Stop(); LuaEvents.Suk_HideEnhancedTechTooltip(); end);
