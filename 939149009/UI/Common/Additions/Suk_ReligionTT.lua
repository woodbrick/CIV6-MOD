...
function UpdateReligionTooltip(tControl, pCity)
	...
	local bReveal = false
	-------------------------------------------------
	if iMajorityReligion ~= -1 and iMajorityReligion ~= GameInfo.Religions["RELIGION_PANTHEON"].Index then
...
function SetReligionTooltip(tControl, pCity)
	...
	tControl:SetToolTipCallback(function() UpdateReligionTooltip(tControl, pCity) end)
end
LuaEvents.Suk_SetReligionTooltip.Add(SetReligionTooltip)