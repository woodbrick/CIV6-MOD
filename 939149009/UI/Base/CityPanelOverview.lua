...
-- SukAddin
----------------------------------------
function SukAddin(tData, tInstance)
	local sType = string.gsub(tData.Icon, 'ICON_', '')

	local bBuilding = false
	local tObj	= GameInfo.Districts[sType]
	if not tObj then
		bBuilding = true
		tObj = GameInfo.Buildings[sType]
	end

	local sToolTip
	if bBuilding then
		sToolTip = ToolTipHelper.GetBuildingToolTip(sType, Game.GetLocalPlayer(), UI.GetHeadSelectedCity())
	else
		sToolTip = ToolTipHelper.GetDistrictToolTip(sType)
	end

	local cost = tObj.Cost or 0;
	if(cost ~= 0) then
		local yield = GameInfo.Yields["YIELD_PRODUCTION"];
		if(yield) then
			sCost = Locale.Lookup("LOC_TOOLTIP_BASE_COST", cost, yield.IconString, yield.Name) .. "[NEWLINE]"
		end

		sToolTip = string.gsub(sToolTip, string.gsub(sCost, "([^%w])", "%%%1"), '')
	end

	tInstance.Top:SetToolTipString(sToolTip)
	tInstance.Button:RegisterCallback( Mouse.eRClick, function()
		LuaEvents.OpenCivilopedia(sType)
	end)
end
...
function ViewPanelBreakdown( data:table )	
		...
		if district.isBuilt then
			...
			for _,building in ipairs(district.Buildings) do
				if building.isBuilt then
					...
					SukAddin(building, kInstanceBuild)
				end
			end

			SukAddin(district, kInstanceDistrict)
	...
	for _, wonder in ipairs(data.Wonders) do
		...
		SukAddin(wonder, kInstanceWonder)
	end
